...
class AirMSPIMeasurements(shdom.Measurements):
    ...
    def imshow(cls, directory, index=-1):
        ...
        for ax, image in zip(axarr, images):
            # image -= image.min()
            ax.imshow((image-image.min()) / image.max())
        return images
    ...
    def load_airmspi_measurements(self, directory):
        ...
        if self.images is not None:
            self._region_of_interest = [0, self.images[0].shape[0], 0, self.images[0].shape[1]]
        if self.wavelength is not None:
            self._valid_wavelength_range = [np.min(self.wavelength), np.max(self.wavelength)]
